...
const fetchReservations = () => async (dispatch) => {
  const response = await fetch('https://handymate-backend-api.onrender.com/api/v1/reservations');
...
const deleteReservation = (reservationId) => async (dispatch) => {
  await fetch(`https://handymate-backend-api.onrender.com/api/v1/reservations/${reservationId}`, {
    method: 'DELETE',
  });
